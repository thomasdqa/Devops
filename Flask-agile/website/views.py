...
def home():
    ...
    if request.method == 'POST':
        ...
        service_request_categoryid = request.form.get(
            'service_request_categoryid')
        ...
        if len(service_request) < 1:
            ...
            new_service_request = Service_request(
                data=service_request, user_id=current_user.id, status_id="1", category_id=service_request_categoryid)
...
def service_id_request():
    ...
    statement = update(Service_request).where(
        Service_request.id == service_requestId).values(status_id=2)
    if service_request:
...
def service_id_request2():
    ...
    # updates the value of status_id on the corrosponding service request
    statement = update(Service_request).where(
        Service_request.id == service_requestId2).values(status_id=3)
    if service_request2:
        db.session.execute(statement)
        db.session.commit()
